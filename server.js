...
app.post("/analyze-image", async (req,res)=>{
 ...
 const ai=await fetch("https://api.openai.com/v1/chat/completions",{
  ...
  body:JSON.stringify({
   ...
   messages:[
    {role:"user",content:[
     {type:"text",text:"Identify object and estimate price"},
...
app.post("/swarm", async (req,res)=>{
 const roles=["research","execute","analyze"];
 let out=[];
 ...
 for(const r of roles){
  const ai=await fetch("https://api.openai.com/v1/chat/completions",{
   ...
   body:JSON.stringify({
    model:"gpt-4o-mini",
    messages:[
     {role:"system",content:`You are ${r}`},
     {role:"user",content:req.body.goal}
    ]
   })
  });

  const d=await ai.json();
  out.push({role:r,text:d.choices[0].message.content});
 }
